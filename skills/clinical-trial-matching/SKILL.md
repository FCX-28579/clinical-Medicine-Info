...
## Supplemental drug report integration

After the base trial matching report has selected ClinicalTrials.gov trials, generate the optional Chinese drug-information supplement with the sibling `supplemental-drug-report` subskill:
...
This keeps the original matching flow unchanged. The supplement reads each selected CT.gov trial to identify the core study drug and sponsor, then matches FDA comparator evidence from the local database built by `fda-drug-evidence-builder`. It does not perform live openFDA searches during report rendering.

Refresh the local FDA evidence database when needed:
...
Updated subskill index additions:

| Subskill | Purpose | When |
|---|---|---|
| [fda-drug-evidence-builder](../fda-drug-evidence-builder/SKILL.md) | Builds the local FDA oncology drug evidence database from openFDA labels | Refreshing or auditing FDA drug evidence |
| [supplemental-drug-report](../supplemental-drug-report/SKILL.md) | Adds Chinese drug-information modules to selected CT.gov trials using the local FDA database | After trial matching has produced decision paths |
